...
t = input_(
    "What do you want to scrape for links today?:\n\
        (G)allery\n\
        (U)ser\n\
        (I)ndividual post\n\
        "
...
class DeviantArtParser(HTMLParser):
    ...
    def feed(self, *args, **kwargs):
        ...
        if not self.FinishedYes:
            print(
                "\033[31;42m Warning: Submission does not have a download button. Outputting low-quality version instead...\033[0;0m",
                file=sys.stderr,
            )
...
def pagination_get(user):
    ...
    while more:
        json = requests.get(
            f"https://www.deviantart.com/_napi/da-user-profile/api/gallery/contents?username={user}&offset={offset}&limit={limit}&all_folder=true"
        ).json()
        more = json["hasMore"]
